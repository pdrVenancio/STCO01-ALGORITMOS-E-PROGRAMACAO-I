// TAD para Árvore Binária de Busca
...
// definição do struct elemento ou no
struct no{
  int valor;
  struct no *esq;
  struct no *dir;
  ...
  if(*raiz == NULL){
    *raiz = novo;
    ...
    while(atual != NULL){
      ...
      // verifica se o valor novo já existe na árvore (árvore não insere elementos repetidos)
...
No* remover_no(No *atual)
{
  ...
  // busca o nó mais a direita da subárvore da esquerda do nó atual, "no2" 
  while(no2->dir != NULL){
