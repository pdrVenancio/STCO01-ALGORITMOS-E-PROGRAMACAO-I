...
int inserir_lista_ordenada(Lista *li, int dado) {
	...
	// percorre a lista, enquanto houver elementos, e o dado da lista for menor
	// que o dado a ser inserido
	int i = 0;
	while (i < li->qtd && li->dados[i] < dado) {
		i++;
	}

	// movimenta uma posição para frente, todos os elementos que estão depois da
	// posição i, onde o novo elemento será inserido
...
int remover_lista_meio(Lista *li, int dado) {
	...
	// percorre a lista, enquanto houver elementos, e o dado da lista for
	// diferente que o dado a ser removido
	int i = 0;
	while (i < li->qtd && li->dados[i] != dado) {
		i++;
	}
	...
	// movimenta uma posição para trás, todos os elementos que estão depois da
	// posição i, onde o elemento foi removido
...
int buscar_lista_posicao(Lista *li, int pos, int *dado) {
	// verifica se a lista foi criada corretamente e se a posição é válida (note
	// que é a posição na lista e não o índice do vetor)
