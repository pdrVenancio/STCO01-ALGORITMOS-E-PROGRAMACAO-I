...
{
    *dado = (Aluno*) malloc(sizeof(Aluno*));  // usa ponteiro para ponteiro pois os valor de dado muda dentro da função
...
{
  // verifica se a lista foi criada corretamente, se não está vazia e se a posição é válida (note que é a posição na lista não é o índice do vetor)
...
int buscar_lista_dado(Lista *li, int matricula, int *pos)
{
  ...
  // procura a posição no vetor que contém um aluno com o número de matrícula desejado
  ...
  // copia a posição da lista onde a matrícula foi encontrada (parâmetro pos passado por referência)
  *pos = i; 
...
int imprimir_lista(Lista *li)
{
    ...
    while(aux != NULL){
      imprime_dado(aux->dado); // imprime todos os campos da estrutura aux->dado do tipo Aluno
      aux = aux->prox;
    }
